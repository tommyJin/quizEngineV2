...
public class QuestionType extends Model<QuestionType> {
    ...
    public Page<QuestionType> query(Map<String, Object> filter){
        ...
        String where = "from "+TableName.question_type+" where 1=1 and isDeleted = "+ConstantParas.isDeleted_false+" ";
        ...
        return QuestionType.dao.paginate(page, size, select, where + order);
    }

    public QuestionType getBy(Map<String,Object> filter){
        String select = "select * ";
        String where = " from "+TableName.question_type+" where 1=1 and isDeleted = "+ConstantParas.isDeleted_false+" ";
        if (filter.get("id")!=null){
            where += " and id = "+Integer.parseInt(filter.get("id").toString());
        ...
        if (!where.equals(" from "+TableName.question_type+" where 1=1 and isDeleted = "+ConstantParas.isDeleted_false+" ")){
            return QuestionType.dao.find(select + where).size()>0?QuestionType.dao.find(select + where).get(0):null;
        }else {
            return null;
        }
    }

    public boolean add(QuestionType question) {
    ...
    public boolean update(QuestionType question){
        Map<String, Object> filter = new HashMap<>();
    ...
    public boolean delete(QuestionType question){
        Map<String, Object> filter = new HashMap<>();
        filter.put("id", question.get("id"));
        if (getBy(filter) != null) {
